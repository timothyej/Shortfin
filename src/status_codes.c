...
status_codes *status_codes_init() {
	...
	memcpy (sc->HTTP_VER, "HTTP/1.1 \0", sc->HTTP_VER_LEN+1);
	...
	memcpy (sc->HTTP_200, "200 OK\0", sc->HTTP_200_LEN+1);
	
	sc->HTTP_302 = malloc(sc->HTTP_302_LEN+1);
	memcpy (sc->HTTP_302, "302 Object Found\0", sc->HTTP_302_LEN+1);

	sc->HTTP_400 = malloc(sc->HTTP_400_LEN+1);
	memcpy (sc->HTTP_400, "400 Bad Request\0", sc->HTTP_400_LEN+1);

	sc->HTTP_404 = malloc(sc->HTTP_404_LEN+1);
	memcpy (sc->HTTP_404, "404 Not Found\0", sc->HTTP_404_LEN+1);

	sc->HTTP_500 = malloc(sc->HTTP_500_LEN+1);
	memcpy (sc->HTTP_500, "500 Internal Error\0", sc->HTTP_500_LEN+1);
